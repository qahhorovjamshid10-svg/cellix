...
import {
  PLAYER, ENEMIES, BOSS_CLASSIC, PLAYER_PROJECTILE, ENEMY_PROJECTILE,
  COLLISION, SCORING, ARENA, DIFFICULTY_CLASSIC,
...
export default class MainScene extends Phaser.Scene {
  private configCallbacks!: MainSceneConfig
  ...
  private challengeSpeedMult: number = 1
  private challengeScoreMult: number = 1
  private challengeHazardFrequencyMult: number = 1
  private challengeEnemyDensityMult: number = 1
  ...
  public init(data: { config: MainSceneConfig }) {
    ...
    this.dailyChallenge = data.config.challenge
    this.challengeSpeedMult = this.dailyChallenge?.modifier.speedMult ?? 1
    this.challengeScoreMult = this.dailyChallenge?.modifier.scoreMult ?? 1
    this.challengeHazardFrequencyMult = this.dailyChallenge?.modifier.hazardFrequencyMult ?? 1
    this.challengeEnemyDensityMult = this.dailyChallenge?.modifier.enemyDensityMult ?? 1
    this.playerMaxHp *= this.dailyChallenge?.modifier.playerHpMult ?? 1
    this.playerHp = this.playerMaxHp
    this.playerSpeed *= this.challengeSpeedMult
    this.playerDamage *= this.dailyChallenge?.modifier.playerDamageMult ?? 1
    this.xpMultiplier *= this.dailyChallenge?.modifier.xpMult ?? 1
  ...
  private createProceduralTextures() {
    ...
    // 8. Enemy: Boss Ancient Cell
    gfx.clear()
    gfx.fillStyle(0xb026ff, 1)
    gfx.fillCircle(45, 45, 40)
    gfx.lineStyle(4, 0xff0055, 1)
    gfx.strokeCircle(45, 45, 40)
    gfx.fillStyle(0xffffff, 0.9)
    gfx.fillCircle(30, 30, 10)
    gfx.generateTexture('enemy_boss', 90, 90)
  ...
  create() {
    ...
    this.hazardManager = new HazardManager(
      this,
      this.dailyChallenge ? createSeededRandom(this.dailyChallenge.seed) : Math.random
    )
    ...
    this.gameTimerEvent = this.time.addEvent({
      ...
      callback: () => {
        this.survivalTime++
        this.score += SCORING.perSecond * this.challengeScoreMult
    ...
    this.time.addEvent({
      delay: DIFFICULTY_CLASSIC.spawnInterval,
      callback: this.spawnEnemyWave,
      callbackScope: this,
      loop: true,
    })

    for (let i = 0; i < ARENA.initialGemCount; i++) {
      this.spawnXpGem(
  ...
  update(time: number) {
    ...
    // Periodic Random Arena Hazard Spawner (every 18 seconds)
    if (time > this.lastHazardSpawnTime + 18000 / this.challengeHazardFrequencyMult) {
      this.lastHazardSpawnTime = time
      const hx = Phaser.Math.Clamp(this.player.x + Phaser.Math.Between(-300, 300), SPAWN_BOUNDS.min, SPAWN_BOUNDS.max)
      const hy = Phaser.Math.Clamp(this.player.y + Phaser.Math.Between(-300, 300), SPAWN_BOUNDS.min, SPAWN_BOUNDS.max)
    ...
    if (this.boss && this.boss.active) {
      ...
      this.boss.setVelocity(Math.cos(angle) * 90, Math.sin(angle) * 90)

      const bossHpPct = this.bossHp / this.bossMaxHp
      const nextBossPhase = bossHpPct <= BOSS_CLASSIC.phases[1] ? 3 : bossHpPct <= BOSS_CLASSIC.phases[0] ? 2 : 1
      if (nextBossPhase > this.bossPhase) {
        this.bossPhase = nextBossPhase
        soundManager.playBossSpawn()
        this.cameras.main.shake(250, 0.012)
        if (this.bossPhase === 2 && !this.bossMinionsSpawned) {
          this.spawnBossMinions()
        }
      }

      const bossAttackCooldown = BOSS_CLASSIC.attackCooldowns[this.bossPhase - 1] || 2200
      if (time > this.lastBossShootTime + bossAttackCooldown) {
        this.fireBossBurstPattern()
        this.lastBossShootTime = time
      }

      if (this.bossPhase === 3 && time > this.lastBossShockwaveTime + BOSS_CLASSIC.shockwaveInterval) {
  ...
  public triggerVoidCoreBlackHole() {
    ...
    const voidX = this.player.x + Phaser.Math.Between(-150, 150)
    const voidY = this.player.y + Phaser.Math.Between(-150, 150)
  ...
  private spawnEnemyWave() {
    const diff = getClassicDifficulty(this.survivalTime, this.playerLevel)
    
    if (this.enemies.countActive() >= diff.enemyCap) return

    const playerX = this.player.x
    const playerY = this.player.y

    const count = Math.ceil(Phaser.Math.Between(diff.groupMin, diff.groupMax) * this.challengeEnemyDensityMult)
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2
      const dist = Phaser.Math.Between(DIFFICULTY_CLASSIC.spawnDistance[0], DIFFICULTY_CLASSIC.spawnDistance[1])
      ...
      let key = 'eater' as keyof typeof ENEMIES
      ...
      const conf = ENEMIES[key]
      const enemy = this.enemies.create(spawnX, spawnY, conf.textureKey) as Phaser.Physics.Arcade.Sprite
      enemy.setActive(true).setVisible(true)
      
      const hp = conf.baseHp + this.playerLevel * conf.hpPerLevel
      enemy.setData('hp', hp)
      enemy.setData('maxHp', hp)
      enemy.setData('speed', conf.speed * this.challengeSpeedMult)
      enemy.setData('xpValue', conf.xpValue)
      enemy.setData('enemyType', conf.textureKey)

      // Elite chance check
      if (Math.random() < diff.eliteChance) {
    ...
    if (this.playerLevel >= DIFFICULTY_CLASSIC.bossMinLevel && (!this.boss || !this.boss.active) && Math.random() > 1 - DIFFICULTY_CLASSIC.bossSpawnChance) {
      this.spawnBossAncientCell()
    }
  }

  private spawnBossAncientCell() {
    soundManager.playBossSpawn()
    ...
    this.bossMaxHp = BOSS_CLASSIC.baseHp + this.playerLevel * BOSS_CLASSIC.hpPerLevel
    ...
    this.boss.setCircle(BOSS_CLASSIC.hitboxRadius)
    this.boss.setData('hp', this.bossHp)
    this.boss.setData('maxHp', this.bossMaxHp)
    this.boss.setData('speed', BOSS_CLASSIC.speed * this.challengeSpeedMult)
    this.boss.setData('xpValue', BOSS_CLASSIC.xpReward)
  ...
  private fireBossBurstPattern() {
    ...
    const projectileCount = BOSS_CLASSIC.burstCounts[phaseIdx] || 8
    const projectileSpeed = BOSS_CLASSIC.burstSpeeds[phaseIdx] || 220
    for (let i = 0; i < projectileCount; i++) {
      ...
      if (spore) {
        spore.setActive(true).setVisible(true)
        spore.setVelocity(Math.cos(angle) * projectileSpeed, Math.sin(angle) * projectileSpeed)
        this.time.delayedCall(3000, () => spore.setActive(false).setVisible(false))
      }
    }
  }
  ...
  private spawnBossMinions() {
    ...
    const minionCount = BOSS_CLASSIC.minionCount || 4
    for (let i = 0; i < minionCount; i++) {
      ...
      const minionHp = BOSS_CLASSIC.minionHpBase + this.playerLevel * BOSS_CLASSIC.minionHpPerLevel
      minion.setData('hp', minionHp)
      minion.setData('maxHp', minionHp)
      minion.setData('speed', i % 2 === 0 ? 150 : 180)
      minion.setData('xpValue', BOSS_CLASSIC.minionXp)
  ...
  private triggerBossShockwave() {
    ...
    this.tweens.add({
      targets: ring,
      radius: BOSS_CLASSIC.shockwaveRadius,
      alpha: 0,
      duration: 700,
      onComplete: () => ring.destroy(),
    })
    ...
    this.time.delayedCall(450, () => {
      ...
      if (distance <= BOSS_CLASSIC.shockwaveRadius) this.damagePlayer(BOSS_CLASSIC.shockwaveDamage)
    })
  }
  ...
  private damageEnemy(enemy: Phaser.Physics.Arcade.Sprite, damageAmount: number) {
    ...
    if (hp <= 0) {
      ...
      this.kills++
      this.score += xpVal * SCORING.enemyKillMult * this.challengeScoreMult
      ...
      if (enemy === this.boss) {
        ...
        this.boss = null
        for (let j = 0; j < BOSS_CLASSIC.deathGemCount; j++) {
          this.spawnXpGem(enemy.x + Phaser.Math.Between(-40, 40), enemy.y + Phaser.Math.Between(-40, 40), BOSS_CLASSIC.deathGemXp)
        }
      }
